<x-app-layout>
    ...
    <div class="py-12 bg-slate-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-t-2xl shadow-lg p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-blue-400/30 relative overflow-hidden">
                <div class="relative z-10">
                    <!-- Tombol Kembali -->
                    <a href="/dashboard" class="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-cyan-50 hover:text-white transition-all bg-black/10 hover:bg-black/30 px-3 py-1.5 rounded-lg border border-white/10 backdrop-blur-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                        Kembali ke Dashboard
                    </a>
                    <h3 class="text-2xl font-bold text-white tracking-wide">Histori Kedisiplinan</h3>
                    <p class="text-blue-100 text-sm mt-1">Pantau riwayat poin dan catatan pelanggaran siswa.</p>
                </div>
                
                <div class="relative z-10">
                    <!-- Tombol Tambah Poin -->
                    <a href="{{ route('catatan-poin.create') }}" class="bg-white/10 hover:bg-white/20 text-white border border-white/30 backdrop-blur-sm font-bold py-2.5 px-6 rounded-xl transition duration-300 shadow-lg flex items-center gap-2">
                    ...
                    <table class="w-full text-sm text-left text-slate-300">
                        <thead class="text-xs uppercase bg-slate-900/50 text-slate-400 border-b border-slate-700">
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-4 font-semibold">Dicatat Oleh</th>
                            ...
                            @forelse ($catatanPoins as $catatanPoin)
                                <tr class="hover:bg-slate-700/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-3 py-1 text-xs font-bold rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-sm">
                                            +{{ $catatanPoin->pelanggaran->poin ?? 0 }} Poin
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2 text-slate-400">
                                            ...
                                            {{ \Carbon\Carbon::parse($catatanPoin->tanggal)->format('d M Y') }}
                                        </div>
                                    </td>
                                    ...
                                    <!-- Keterangan -->
                                    <td class="px-6 py-4 text-slate-400 max-w-xs truncate" title="{{ $catatanPoin->keterangan ?? '-' }}">
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-slate-500">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="font-medium text-slate-400">Belum ada histori catatan poin.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
