...
<head>
    ...
    <title>Daftar Akun — Ruang Aman Siswa</title>
...
<body class="bg-slate-900 text-slate-100 antialiased">
    ...
    <div class="relative min-h-screen w-full overflow-hidden lg:grid lg:grid-cols-[55%_45%] xl:grid-cols-[60%_40%]">
        ...
        {{-- ================= LEFT — BRANDING PANEL ================= --}}
        <div class="relative hidden min-h-screen flex-col justify-center overflow-hidden border-r border-slate-700 bg-slate-900 px-8 py-12 lg:flex lg:px-16 xl:px-24">
            ...
            <div class="relative z-10 flex flex-col items-start gap-6">
                ...
                <p class="mt-2 max-w-md text-base sm:text-lg leading-relaxed text-slate-300 font-medium">
                    Langkah pertamamu menuju lingkungan sekolah yang lebih aman dan suportif dimulai di sini.
                </p>
            </div>
        </div>

        {{-- ================= RIGHT — REGISTER PANEL ================= --}}
        <div class="relative flex min-h-screen items-center justify-center bg-slate-900/80 px-4 py-8 sm:px-10 lg:px-12">
            
            <div class="pointer-events-none absolute right-[-4rem] top-1/4 h-72 w-72 rounded-full bg-blue-500/10 blur-[100px]"></div>

            <div class="relative z-10 w-full max-w-[440px]" x-data="{ showPassword: false, showConfirm: false, loading: false }">
                
                {{-- Form Card --}}
                <div class="rounded-3xl border border-slate-700 bg-slate-800 p-8 shadow-2xl sm:p-10">
                    
                    <div class="mb-6 text-center">
                        <h2 class="font-display text-2xl font-bold tracking-tight text-white sm:text-3xl">Buat Akun Baru</h2>
                        <p class="mt-2 text-sm text-slate-400">Lengkapi data diri kamu di bawah ini.</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('register') }}" @submit="loading = true" class="space-y-4">
                        ...
                        {{-- Email / NIS --}}
                        <div>
                            <label for="email" class="mb-1.5 block text-sm font-medium text-slate-300">Email atau NIS</label>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <button type="button" @click="showConfirm = !showConfirm" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-800">
                                    <svg x-show="!showConfirm" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    <svg x-show="showConfirm" style="display:none;" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                                </button>
                            </div>
                            @error('password_confirmation') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                        </div>

                        {{-- Tombol Daftar --}}
                        <button type="submit" :disabled="loading" class="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 px-4 py-3.5 text-base font-bold text-white shadow-lg shadow-cyan-500/20 transition-all hover:from-blue-500 hover:to-cyan-400 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-70 disabled:cursor-not-allowed">
                    ...
                    <div class="mt-6 text-center">
                        <p class="text-sm text-slate-400">
                            Sudah punya akun? 
                            <a href="{{ route('login') }}" class="font-semibold text-cyan-400 transition-colors hover:text-cyan-300">
                                Masuk di sini
                            </a>
                        </p>
                    </div>
