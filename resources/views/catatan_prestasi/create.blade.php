<x-app-layout>
    ...
    <div class="py-12 bg-slate-900 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-t-2xl shadow-lg p-6 sm:p-8 relative overflow-hidden">
                <div class="relative z-10">
                    <a href="{{ route('catatan-prestasi.index') }}" class="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-emerald-50 hover:text-white transition-all bg-black/10 hover:bg-black/30 px-3 py-1.5 rounded-lg border border-white/10 backdrop-blur-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                        Kembali ke Histori
                    </a>
                    <h3 class="text-2xl font-bold text-white tracking-wide">Catat Poin Prestasi</h3>
                    <p class="text-emerald-100 text-sm mt-1">Cari siswa dan pilih jenis prestasi untuk mencatat poin.</p>
                </div>
            ...
            <div class="bg-slate-800 rounded-b-2xl shadow-xl p-6 sm:p-8 border-x border-b border-slate-700">
                ...
                <form action="{{ route('catatan-prestasi.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="siswa_search" autocomplete="off" placeholder="Ketik untuk mencari nama siswa..."
                               ...
                               class="block w-full rounded-xl border-slate-600 bg-slate-900/60 text-white placeholder-slate-500 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                        ...
                        <div id="siswa_dropdown"
                            ...
                            @foreach ($siswas as $siswa)
                                <div class="siswa-option px-4 py-2 text-sm text-slate-100 hover:bg-emerald-600 cursor-pointer"
                    ...
                    <div>
                        <label for="prestasi_id" class="block text-sm font-semibold text-slate-300 mb-1">Jenis Prestasi</label>
                        <select id="prestasi_id" name="prestasi_id"
                                class="block w-full rounded-xl border-slate-600 bg-slate-900/60 text-white shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                            <option value="">-- Pilih Jenis Prestasi --</option>
                            @foreach ($jenisPrestasis as $jenisPrestasi)
                                <option value="{{ $jenisPrestasi->id }}" {{ old('prestasi_id') == $jenisPrestasi->id ? 'selected' : '' }}>
                                    {{ $jenisPrestasi->nama_prestasi }} (-{{ $jenisPrestasi->poin }} poin)
                                </option>
                            @endforeach
                        </select>
                        @error('prestasi_id')
                    ...
                    <div>
                        ...
                        <textarea id="keterangan" name="keterangan" rows="3"
                                  class="block w-full rounded-xl border-slate-600 bg-slate-900/60 text-white shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">{{ old('keterangan') }}</textarea>
                        @error('keterangan')
                            <p class="mt-1 text-sm text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2">
                        <a href="{{ route('catatan-prestasi.index') }}" class="text-sm text-slate-400 hover:text-slate-200">
                            Batal
                        </a>
                        <button type="submit"
                                class="bg-gradient-to-r from-emerald-500 to-teal-600 hover:opacity-90 text-white font-bold py-2.5 px-6 rounded-xl transition duration-300 shadow-lg text-sm">
                            Simpan Poin Prestasi
                        </button>
                    </div>
                </form>
            </div>
