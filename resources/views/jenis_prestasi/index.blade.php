        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-gradient-to-r from-emerald-500 to-teal-600 rounded-t-2xl shadow-lg p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative overflow-hidden">
                <div class="relative z-10">
                    ...
                    <h3 class="text-2xl font-bold text-white tracking-wide">Master Jenis Prestasi</h3>
                    <p class="text-emerald-100 text-sm mt-1">Kelola daftar prestasi siswa dan bobot poin pengurangnya.</p>
                </div>
                <div class="relative z-10">
                    <a href="{{ route('jenis-prestasi.create') }}" class="bg-white/10 hover:bg-white/20 text-white border border-white/30 backdrop-blur-sm font-bold py-2.5 px-6 rounded-xl transition duration-300 shadow-lg flex items-center gap-2">
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-700">
                    <table class="w-full text-sm text-left text-slate-300">
                        <thead class="text-xs uppercase bg-slate-900/50 text-slate-400 border-b border-slate-700">
                            <tr>
                                <th class="px-6 py-4 font-semibold w-1/2">Nama Prestasi</th>
                        ...
                        <tbody class="divide-y divide-slate-700/50">
                            @forelse ($jenisPrestasis as $jenisPrestasi)
                                <tr class="hover:bg-slate-700/30 transition-colors">
                                    <td class="px-6 py-4 font-bold text-white">
                                        {{ $jenisPrestasi->nama_prestasi }}
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full border
                                            @if ($jenisPrestasi->kategori === 'akademik') bg-blue-500/10 text-blue-400 border-blue-500/20
                                            @elseif ($jenisPrestasi->kategori === 'non_akademik') bg-teal-500/10 text-teal-400 border-teal-500/20
                                            @else bg-emerald-500/10 text-emerald-400 border-emerald-500/20
                                            @endif">
                                            {{ str_replace('_', ' ', ucfirst($jenisPrestasi->kategori)) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 flex justify-center gap-3">
                                        <a href="{{ route('jenis-prestasi.edit', $jenisPrestasi) }}"
                                           class="text-blue-400 hover:text-blue-300 transition-colors font-medium text-sm">
                                            Edit
                                        </a>
                                        <form action="{{ route('jenis-prestasi.destroy', $jenisPrestasi) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus jenis prestasi ini?');">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="font-medium text-slate-400">Belum ada daftar jenis prestasi.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
